<template>
  <Popover>
    <PopoverTrigger>
      <Button variant="ghost" class="text-white text-[14px] p-2 flex gap-2 font-medium">
        <FontAwesome :icon="['fas', 'location-dot']" class="h-6 w-6"/>
        informe seu CEP
      </Button>
    </PopoverTrigger>

    <PopoverContent align="start" class="min-w-max">
          ...
          <FormControl >
            <div class="w-max flex flex-row items-center gap-6">
              <PinInput @update:model-value="value => setFieldValue('code', value)" >
                <PinInputGroup>
                  <PinInputInput v-for="(id, index) in 8" :key="id" :index="index"/>
                </PinInputGroup>
              </PinInput>
